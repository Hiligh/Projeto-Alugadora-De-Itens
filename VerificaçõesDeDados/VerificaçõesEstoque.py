...
#verifica se tem escoras disponiveis a serem usadas
def verificaçãoEstoque(quant: int, tipo: str, tipoCampo: str):
    el = db.all()
    verificacao = 0
    for i in range(len(db)):
        escoras = el[i]
        verificacao += int(escoras[tipo])
    if(quant - verificacao == 0):
        resultado = criaBoxDeErro(tipoCampo)
        return resultado
    else:
        return 3 #retorna 3 pois nao pode retornar nem 1 ou 2. por ser opçoes no criaBoxErro
